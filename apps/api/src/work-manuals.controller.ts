...
▶ 반려 루프백 규칙:
- 결재(APPROVAL) 태스크 뒤에 반려 시 이전 작성 태스크로 돌아가야 하는 경우, 결재 태스크 → 작성 태스크로 향하는 edge를 추가하고 isLoopBack: true로 설정하세요.
...
- 결재 반려 시 이전 작성 step으로 루프백이 필요하면 edge에 isLoopBack: true와 condition: "last.approval.status == 'REJECTED'" 추가
