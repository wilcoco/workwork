...
3. 결재: 결재가 필요한 단계와 결재선 — 누가, 어떤 순서로 결재하는지 (예: 팀장 → 공장장). 매뉴얼에 결재 단계가 있는데 결재선이 불명확하면 반드시 질문하세요.
4. 분기: 조건에 따라 갈라지는 지점과 그 조건 (예: 금액 기준, 합격/불합격)
5. 기한: 각 단계가 프로세스 시작 후 며칠 이내에 완료되어야 하는지. 기한이 없는 단계가 있으면 반드시 질문하세요 (여러 단계를 묶어 한 번에 물어도 됩니다. 예: "신청서 작성, 검사, 고객 회신은 각각 시작 후 며칠 이내에 끝나야 합니까?")
...
누락이 없으면 { "gaps": [] }`;
  ...
  async aiBpmn(@Param('id') id: string, @Body() dto: AiBpmnDto) {
    ...
    // 조직 목록 로드: AI가 담당 부서를 실제 조직으로 매핑할 수 있게 한다
    const orgUnitsAll = await this.prisma.orgUnit.findMany({ select: { id: true, name: true }, orderBy: { name: 'asc' } });
    const orgUnits = orgUnitsAll.filter((o) => !/^personal\s*-/i.test(o.name || '')).slice(0, 200);
    const normOrgName = (s: string) => String(s || '').toLowerCase().replace(/\s+/g, '');
    const orgByName = new Map(orgUnits.map((o) => [normOrgName(o.name), o.id]));
    const orgListText = orgUnits.map((o) => o.name).join(', ');
...
출력 JSON 스키마:
{
  ...
  "bpmnJson": {
    ...
    "edges": Array<{ id: string, source: string, target: string, condition?: string, isLoopBack?: boolean }>
  }
}
...
- AND-Join: 병렬 태스크들이 모두 끝나야 하는 합류점에 다시 gateway_parallel 노드를 두고, 각 병렬 task → join gateway → 후행 task로 edge를 연결하세요.
...
▶ 결재선 규칙:
- APPROVAL 태스크의 assigneeHint에는 결재선(결재자와 순서)을 그대로 기록하세요. 예: "경영관리팀장 → 공장장", "팀장 전결"
- 매뉴얼이나 질의응답에 결재선이 언급되어 있으면 절대 누락하지 마세요.

▶ 결재 분기(승인/반려) 규칙:
- 결재(APPROVAL) 태스크에서 나가는 edge는 승인/반려 분기를 항상 명시하세요.
- 승인 후 진행하는 전진 edge: condition: "last.approval.status == 'APPROVED'"
- 반려 시 이전 작성 태스크로 돌아가는 edge: isLoopBack: true + condition: "last.approval.status == 'REJECTED'"
- 루프백은 결재 반려 패턴에서만 사용하세요.
- 예시: 작성(TaskA) → 결재(TaskB) → [APPROVED: TaskC 전진, REJECTED(isLoopBack): TaskA 복귀]

▶ 담당 조직(assigneeOrgUnitName) 규칙:
- 메뉴얼에서 해당 단계의 담당 부서/팀을 알 수 있으면, assigneeOrgUnitName에 아래 [조직 목록] 중 하나를 글자 그대로 설정하세요.
- 목록에 없는 조직명을 새로 만들지 마세요. 어느 조직인지 불확실하면 assigneeOrgUnitName은 생략하세요.
- assigneeHint에는 메뉴얼에 적힌 담당 표현(예: "구매 담당자", "품질팀 검사원")을 그대로 기록하세요. 매핑 여부와 무관하게 항상 채우는 것을 권장합니다.
...
- STEP 블록의 목적/입력/산출물/업무일지/완료조건은 description에 요약(HTML)`;
    ...
    const bpmnToolSchema = {
      ...
      properties: {
        ...
        bpmnJson: {
          type: 'object',
          properties: {
            nodes: { type: 'array', items: { type: 'object', properties: { id: { type: 'string' }, type: { type: 'string', enum: ['start', 'end', 'task', 'gateway_xor', 'gateway_parallel'] }, name: { type: 'string' }, taskType: { type: 'string', enum: ['WORKLOG', 'COOPERATION', 'APPROVAL'] }, description: { type: 'string' }, assigneeHint: { type: 'string' }, assigneeOrgUnitName: { type: 'string' }, stageLabel: { type: 'string' }, deadlineOffsetDays: { type: 'number' }, slaHours: { type: 'number' } }, required: ['id', 'type', 'name'] } },
            edges: { type: 'array', items: { type: 'object', properties: { id: { type: 'string' }, source: { type: 'string' }, target: { type: 'string' }, condition: { type: 'string' }, isLoopBack: { type: 'boolean' } }, required: ['id', 'source', 'target'] } },
          },
    ...
    const normalizedNodes = (nodes as any[]).map((n: any) => {
      ...
      const out: any = { ...n, taskType: nextTt };
      // Normalize deadlineOffsetDays/slaHours
      if (out.deadlineOffsetDays != null) {
        const d = Number(out.deadlineOffsetDays);
        out.deadlineOffsetDays = (Number.isFinite(d) && d > 0) ? Math.round(d) : undefined;
      }
      if (out.slaHours != null) {
        const h = Number(out.slaHours);
        out.slaHours = (Number.isFinite(h) && h > 0) ? Math.round(h) : undefined;
      }
      if (out.stageLabel != null) out.stageLabel = String(out.stageLabel).trim() || undefined;
      // AI가 매핑한 조직명을 실제 OrgUnit ID로 해석 → 인스턴스 시작 시 자동 담당자 배정 가능
      const orgName = String(out.assigneeOrgUnitName || '').trim();
      if (orgName) {
        const oid = orgByName.get(normOrgName(orgName));
        if (oid) {
          out.assigneeType = 'ORG_UNIT';
          out.assigneeOrgUnitId = oid;
        }
        if (!out.assigneeHint) out.assigneeHint = orgName;
      }
      delete out.assigneeOrgUnitName;
      return out;
    });
    // Normalize loopBack edges
    const normalizedEdges = (edges as any[]).map((e: any) => {
      if (e.isLoopBack) {
        e.isLoopBack = Boolean(e.isLoopBack);
        if (e.isLoopBack && !e.condition) e.condition = "last.approval.status == 'REJECTED'";
      }
      return e;
    });

    return { title: outTitle, bpmnJson: { ...bpmnJson, nodes: normalizedNodes, edges: normalizedEdges }, aiModel };
  }
...
- 병렬 실행 가능한 step 그룹이 있으면 gateway_parallel (AND-Split/Join) 사용
...
- 결재(APPROVAL) step에서 나가는 edge는 승인/반려 분기를 명시: 전진 edge에 condition: "last.approval.status == 'APPROVED'", 반려 복귀 edge에 isLoopBack: true + condition: "last.approval.status == 'REJECTED'"
- step에 기한 정보가 있으면 deadlineOffsetDays(D+N일) 또는 slaHours(N시간) 설정
- 논리적 단계 구분이 있으면 stageLabel 설정 (예: "1. 기획", "2. 설계")
- step의 actor(담당)가 아래 [조직 목록]의 조직과 대응되면 assigneeOrgUnitName에 목록의 이름을 글자 그대로 설정하세요. 목록에 없는 이름은 만들지 말고, 불확실하면 생략하세요. assigneeHint에는 actor 표현을 그대로 기록하세요.
...
출력 JSON:
{
  "title": string,
  "bpmnJson": {
    "nodes": Array<{ id, type, name, taskType?, description?, assigneeHint?, assigneeOrgUnitName?, stageLabel?, deadlineOffsetDays?, slaHours? }>,
    ...
    const bpmnSchema = {
      ...
      properties: {
        ...
        bpmnJson: {
          type: 'object',
          properties: {
            nodes: { type: 'array', items: { type: 'object', properties: { id: { type: 'string' }, type: { type: 'string' }, name: { type: 'string' }, taskType: { type: 'string' }, description: { type: 'string' }, assigneeHint: { type: 'string' }, assigneeOrgUnitName: { type: 'string' }, stageLabel: { type: 'string' }, deadlineOffsetDays: { type: 'number' }, slaHours: { type: 'number' } }, required: ['id', 'type', 'name'] } },
            edges: { type: 'array', items: { type: 'object', properties: { id: { type: 'string' }, source: { type: 'string' }, target: { type: 'string' }, condition: { type: 'string' }, isLoopBack: { type: 'boolean' } }, required: ['id', 'source', 'target'] } },
          },
          required: ['nodes', 'edges'],
        },
    ...
    // Normalize taskTypes and new fields
    ...
    const normalizedEdges = (bpmnJson.edges as any[]).map((e: any) => {
      if (e.isLoopBack) {
        e.isLoopBack = Boolean(e.isLoopBack);
        if (e.isLoopBack && !e.condition) e.condition = "last.approval.status == 'REJECTED'";
      }
