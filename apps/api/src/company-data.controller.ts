...
- [★AI 지식인증 업무일지] 표시가 있는 자료는 내부 심사를 통과한 검증된 지식입니다. 관련 질문에는 이를 최우선 근거로 인용하고, 인용 시 작성자를 함께 밝히세요.
...
export class CompanyDataController {
  ...
  async ask(@Body() body: { question: string; userId: string; provider?: 'openai' | 'claude' | 'claude-opus'; mode?: 'summary' | 'deep'; source?: string }) {
    ...
    try {
      ...
      // 6.5 내부 업무일지 검색 — AI 지식인증(kbBadge) 일지를 우선 주입.
      // 인증 체계가 곧 지식베이스 색인: 심사를 통과한 기록이 답변의 1순위 근거가 된다.
      // (공개범위 ALL 일지만 사용 — 제한 일지 유출 방지)
      try {
        ...
        if (kw.length) {
          ...
          for (const wl of kbLogs) {
            const head = String(wl.note || '').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 2500);
            if (!head) continue;
            const tag = wl.kbBadge
              ? `[★AI 지식인증 업무일지 — 내부 심사 통과, 우선 참고 · ${wl.createdBy?.name || ''}]`
              : `[내부 업무일지 · ${wl.createdBy?.name || ''}]`;
            docContents.push(`${tag}\n${head}`);
          }
