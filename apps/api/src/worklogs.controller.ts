...
출력 JSON: { "questions": [{ "id": number, "question": string }] }`;
  ...
  async aiKbReview(@Param('id') id: string, @Body() body: { userId?: string }) {
    ...
    // 성실 답변 기준 (대표 지시 2026-07-28): 질문에 모두 성실히 답하면 인정 — 박하게 굴지 않는다.
    const sys = `당신은 회사 지식 기록을 격려하는 심사관입니다. 반드시 JSON만 출력하세요.
🏅 인증의 목적은 "성실하게 지식을 남기는 문화"를 키우는 것입니다. 완벽한 지식이 아니라 성실한 답변을 인정하세요.

합격 기준 — [보완 문답]이 있는 경우 (이 기준이 우선):
다음 3가지를 충족하면 합격입니다:
1. 모든 질문에 답했다 (비워둔 질문이 없다)
2. 각 답변이 단답형이 아니다 — "네/아니오/없음/모름/특이사항 없음" 류의 회피성 답이 아니라, 자기 경험·상황이 담긴 문장이다
3. 각 답변이 질문이 기대한 내용을 담고 있다 — 원인을 물으면 원인을, 기준을 물으면 기준을, 재발방지를 물으면 대책을 실제로 말했다
→ 3가지가 충족되면 합격시키세요. 답변이 짧아도 요지에 맞고 실질 내용이 있으면 인정합니다. 애매하면 합격 쪽으로 판단하세요.
→ 일부 질문만 회피성으로 답하고 나머지는 성실하면, 성실한 답이 과반일 때 합격시키세요.

합격 기준 — [보완 문답]이 없는 경우:
본문 자체에 재사용 지식 요소(원인 분석 / 판단 기준·수치 / 따라할 수 있는 절차 / 재발방지책 / 주의점) 중 1가지 이상이 구체적으로 담겨 있으면 합격.

불합격시켜야 하는 경우만:
- 모든 답변이 회피성 한두 단어이거나 질문을 되풀이한 경우
- 문답도 없고 본문도 단순 완료 나열("~했음")뿐인 경우

출력 JSON: { "awarded": boolean, "reason": string, "hint": string }
- awarded=true: reason에 어떤 점이 좋았는지 격려하는 한 문장, hint는 빈 문자열
- awarded=false: reason은 빈 문자열, hint에 "무엇을 보완하면 인증받을 수 있는지" 부드러운 한 문장`;
    const text = `[본문]\n${noteText}\n\n[보완 문답]\n${supText || '(없음)'}`;
