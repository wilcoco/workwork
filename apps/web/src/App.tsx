...
function getPageGuide(path: string, SHOW_APPROVALS: boolean, SHOW_COOPS: boolean): { title: string; items: string[] } | null {
  ...
  if (path.startsWith('/okr') || path === '/me/goals' || path === '/okr-map' || path.startsWith('/goals')) {
    return {
      title: '목표관리',
      items: [
        '입력 순서: ① 대표가 회사 최상단 목표 정의 → ② 임원이 대표 KR을 받아 실 목표 전개 → ③ 팀장이 팀 OKR 전개 + 정량 목표(팀 KPI) 등록·담당자 지정 → ④ 담당자가 업무일지로 실적 입력. 자세한 안내는 "입력 가이드" 메뉴를 참고하세요.',
...
{(() => { const url = String((import.meta as any)?.env?.VITE_POWERAPPS_APPROVAL_URL || '').trim(); return url ? <a href={url} target="_blank" rel="noopener noreferrer" style={{ marginLeft: 12 }}>내부결재</a> : null; })()}
        {(() => { const url = String((import.meta as any)?.env?.VITE_SELFSERVICE_URL || 'https://selfservice.icams.co.kr').trim(); return url ? <a href={url} target="_blank" rel="noopener noreferrer" style={{ marginLeft: 12 }}>셀프서비스</a> : null; })()}
