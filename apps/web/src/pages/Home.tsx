                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <div style={{ display: 'grid', gap: 4, flex: 1, minWidth: 0 }}>
                              <div style={{ display: 'grid', gap: 2 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  <div style={{ fontWeight: 800, color: '#dc2626' }}>{w.title || '(제목 없음)'}{(w as any).kbBadge ? <><KbBadge note={(w as any).kbBadgeNote} count={(w as any).authorKbCount} /><KbAuthorCount count={(w as any).authorKbCount} /></> : null}</div>
                                  {viewMode === 'summary' ? (
                        ...
                        <div key={w.id} style={{ border: '1px solid #e5e7eb', borderRadius: 10, padding: 10, display: 'grid', gap: 8, background: '#FFFFFF', cursor: 'pointer' }} onClick={() => setDetail(anyW)}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <div style={{ display: 'grid', gap: 4, flex: 1, minWidth: 0 }}>
                              <div style={{ display: 'grid', gap: 2 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  <div style={{ fontWeight: 700 }}>{w.title || '(제목 없음)'}{(w as any).kbBadge ? <><KbBadge note={(w as any).kbBadgeNote} count={(w as any).authorKbCount} /><KbAuthorCount count={(w as any).authorKbCount} /></> : null}</div>
                                  {viewMode === 'summary' ? (
                                    <UserAvatar userId={authorId} name={authorName || w.title} size={22} style={{ marginLeft: 'auto' }} />
                                  ) : null}
                                </div>
