...
export function Home() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => void (async () => {
      ...
      // 결재 소식: 내가 참여한 결재의 의견/최종결과 알림 (안 읽은 것)
      try {
        ...
        <div style={{ background: 'linear-gradient(135deg, #fef3c7 0%, #fde68a 100%)', border: '1px solid #f59e0b', borderRadius: 12, padding: 14, display: 'grid', gap: 10 }}>
          ...
          {approvalNotices.length > 0 && (
            <div style={{ background: '#faf5ff', border: '1px solid #d8b4fe', borderRadius: 8, padding: 10 }}>
              ...
              <div style={{ display: 'grid', gap: 4 }}>
                {approvalNotices.slice(0, 3).map((n: any) => {
                  const p = n.payload || {};
                ...
                {approvalNotices.length > 3 && (
                  <div style={{ fontSize: 12, color: '#9333ea' }}>외 {approvalNotices.length - 3}건 더... (알림함에서 확인)</div>
                )}
              </div>
            </div>
          )}
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <div style={{ display: 'grid', gap: 4, flex: 1, minWidth: 0 }}>
                              <div style={{ display: 'grid', gap: 2 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  <div style={{ fontWeight: 800, color: '#dc2626' }}>{w.title || '(제목 없음)'}{(w as any).kbBadge ? <><KbBadge note={(w as any).kbBadgeNote} count={(w as any).authorKbSeq} /><KbAuthorCount count={(w as any).authorKbSeq} /></> : null}</div>
                                  {viewMode === 'summary' ? (
                        ...
                        <div key={w.id} style={{ border: '1px solid #e5e7eb', borderRadius: 10, padding: 10, display: 'grid', gap: 8, background: '#FFFFFF', cursor: 'pointer' }} onClick={() => setDetail(anyW)}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <div style={{ display: 'grid', gap: 4, flex: 1, minWidth: 0 }}>
                              <div style={{ display: 'grid', gap: 2 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  <div style={{ fontWeight: 700 }}>{w.title || '(제목 없음)'}{(w as any).kbBadge ? <><KbBadge note={(w as any).kbBadgeNote} count={(w as any).authorKbSeq} /><KbAuthorCount count={(w as any).authorKbSeq} /></> : null}</div>
                                  {viewMode === 'summary' ? (
                                    <UserAvatar userId={authorId} name={authorName || w.title} size={22} style={{ marginLeft: 'auto' }} />
                                  ) : null}
                                </div>
